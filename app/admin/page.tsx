...
import { Package, ShoppingCart, Plus, Home, ChevronRight } from 'lucide-react'
import { produtos } from '@/data/produtos'

export default function AdminDashboard() {
  const total = produtos.length
  const disponiveis = produtos.filter(p => p.estoque === 'disponivel').length
  ...
  const acoes = [
    {
      ...
      emoji: '📸',
      titulo: 'Novo produto',
      sub: 'Adicione com foto da câmera',
      cor: 'border-rosa/40 hover:border-rosa hover:bg-rosa/5',
    },
    {
      href: '/admin/home',
      emoji: '🏠',
      titulo: 'Configurar home',
      sub: 'Lançamentos, banners, categorias',
      cor: 'border-vinho/30 hover:border-vinho hover:bg-vinho/5',
    },
    {
      href: '/admin/produtos',
      emoji: '📦',
      titulo: 'Ver produtos',
      sub: `${total} produtos cadastrados`,
      cor: 'border-oliva/30 hover:border-oliva hover:bg-oliva/5',
    },
    {
      href: '/admin/pedidos',
      emoji: '🛍️',
      titulo: 'Pedidos',
      sub: 'Acompanhar vendas',
      cor: 'border-bege/60 hover:border-bege hover:bg-bege/10',
    },
  ]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { label: 'Produtos', value: total,       emoji: '📦' },
          { label: 'Disponíveis', value: disponiveis, emoji: '✅' },
          { label: 'Novos', value: novos,          emoji: '🆕' },
        ].map(s => (
