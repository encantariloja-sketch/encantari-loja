      ...
      <header className={`sticky top-0 z-50 bg-white transition-shadow ${scrolled ? 'shadow-sm' : ''}`}>
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16 md:h-20 gap-4">
            ...
            <div className="flex items-center gap-1 md:gap-2">
              ...
              <Link href="/conta" className="p-2.5 text-vinho/70 hover:text-vinho transition-colors" aria-label="Minha conta">
                <User size={21} />
              </Link>
              <Link href="/carrinho" className="relative p-2.5 text-vinho/70 hover:text-vinho transition-colors" aria-label="Carrinho">
                <ShoppingBag size={21} />
                {totalItens > 0 && (
                  <span className="absolute top-1 right-1 min-w-[18px] h-[18px] bg-rosa text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1">
                    {totalItens > 9 ? '9+' : totalItens}
                  </span>
                )}
              </Link>
              <button onClick={() => setMenuAberto(!menuAberto)} className="md:hidden p-2.5 text-vinho" aria-label="Menu">
        ...
        {menuAberto && (
          <div className="md:hidden fixed inset-0 top-0 z-40 bg-black/40" onClick={() => setMenuAberto(false)}>
            <div className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-white shadow-xl flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-5 border-b border-gray-100">
                <Image src="/logo-escura.png" alt="Encantari" width={120} height={50} className="h-8 w-auto" />
                <button onClick={() => setMenuAberto(false)} className="p-2 text-vinho/60"><X size={20} /></button>
              </div>
              <nav className="flex-1 overflow-y-auto py-4">
                {[{ label: '👤 Minha conta', href: '/conta' }, ...navLinks].map(item => (
